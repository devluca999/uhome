...
export function LeaseMessagesTab({
  lease,
  isLeaseActive,
  defaultIntent = 'general',
  leaseStatus,
}: LeaseMessagesTabProps) {
  const { messages, loading, sendMessage, refetch } = useLeaseMessages(lease.id)
  const { user } = useAuth()
            ...
            {messages.length === 0 ? (
              <div className="flex-1 flex items-center justify-center">
                <EmptyState
                  ...
                  description="Start the conversation with your tenant."
                />
              </div>
            ) : (
              <div className="flex-1 overflow-y-auto space-y-4 pb-4">
                {messages.map(message => (
                  <MessageBubble
                    key={message.id}
                    message={message as Message & { sender?: { email: string | null } }}
