...
async function executeQuery(url: string, serviceKey: string, query: string): Promise<any[]> {
  try {
  ...
  for (const tableName of knownTables) {
    try {
      ...
      if (!colError && columnData) {
        ...
        columns = columnData.map((row: any) => row.column_name).sort()
      } else {
...
async function verifyEnvironment(envName: string, url: string, serviceKey: string) {
  ...
  for (const [tableName, info] of Object.entries(schema)) {
    if ((info as any).exists) {
      const colCount = (info as any).columns?.length || 0
      totalColumns += colCount
      if (colCount > 0) {
        log(`   ${tableName}: ${colCount} columns, ${(info as any).rowCount} rows`, 'blue')
      }
    }
  }
...
async function compareSchemas(staging: any, production: any) {
  log(`\n${'='.repeat(60)}`, 'magenta')
